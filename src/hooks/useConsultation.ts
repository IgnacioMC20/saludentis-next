    ...
    return useMutation<CreateConsultationResponse, Error, IConsultation>(
        {
            ...
            onSuccess: (data) => {
                // Invalidate all consultations queries to refetch data
                queryClient.invalidateQueries({ queryKey: ['consultations'] })
                
                // Invalidate specific patient consultations and balance queries
                if (data.data?.patientId) {
                    const patientId = data.data.patientId.toString()
                    queryClient.invalidateQueries({ queryKey: ['consultations', patientId] })
                    queryClient.invalidateQueries({ queryKey: ['balance', patientId] })
                }
            }
        }
    )
